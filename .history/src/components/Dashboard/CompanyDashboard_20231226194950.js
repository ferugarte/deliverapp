...
const CompanyDashboard = () => {
  ...
  const formatDate = (date) => {
    // Si 'createdAt' es un timestamp, usa new Date(date) para convertirlo
    return new Date(date).toLocaleString(); // Formatea la fecha y hora
  };
          ...
          <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column' }}>
            ...
            <List>
              {latestOrders.map((order, index) => (
                <ListItem key={index}>
                  <ListItemText primary={`Pedido ID: ${order.id}`} secondary={`Fecha: ${formatDate(order.createdAt)}`} />
                </ListItem>
              ))}
          </List>
          </Paper>
