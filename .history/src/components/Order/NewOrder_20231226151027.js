...
const NewOrder = () => {
  ...
  const assignOrderToDriver = async (newOrderData) => {
    ...
    driversSnapshot.forEach(doc => {
      ...
      if (driverData.assignedOrdersCount < minAssignedDriver.assignedOrdersCount || driverData.assignedOrdersCount === undefined) {
    ...
    if (minAssignedDriver.id) {
      ...
      await updateDoc(driverDocRef, {
        assignedOrdersCount: minAssignedDriver.assignedOrdersCount + 1
      });
    } else {
